  ...
  const handleVinChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (decodedData?.make && decodedData?.model && decodedData?.year) {
        ...
        if (!matchedMake) {
          setVinError(`Make "${decodedData.make}" not found in the available makes.`);
          return;
        }
        ...
        if (!matchedModel) {
          setVinError(`Model "${decodedData.model}" not found in the available models.`);
          return;
        }
  ...
  return (
    <Dialog>
      <DialogTrigger asChild>
        <Button className="absolute top-[90px] right-[33px] w-[180px] h-[40px] rounded-lg bg-[#403C89] text-white font-dm-sans font-bold text-sm leading-[140%]">
          + Add New Vehicle
        </Button>
      </DialogTrigger>
      <DialogContent className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[460px] bg-white rounded-[12px] px-[24px] py-[20px] shadow-md">
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-2 gap-x-3 gap-y-2.5 mt-2.5">
            ...
            },
            {
              ...
              component: (
                <div>
                  ...
                  />
                  {isLoadingVin && (
                    <p className="text-xs text-gray-500 mt-1">Decoding VIN...</p>
                  )}
                </div>
              ),
            },
          ...
          <div className="col-span-2 relative">
            <Label htmlFor="location" className="text-[#636777] font-dm-sans font-medium text-[13px] leading-[140%]">
              Location
            </Label>
            <Input
              id="location"
          ...
          <div className="col-span-2">
            <Label htmlFor="zip" className="text-[#636777] font-dm-sans font-medium text-[13px] leading-[140%]">
              Zip Code
            </Label>
            <Input
              id="zip"
